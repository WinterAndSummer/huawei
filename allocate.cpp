...
void allocate_simple(vector<string> vflavors, vector<int> vflavors_pridict_nums, string dim, char* filename)
{
	...
	for (int i = vflavors.size() - 1; i >= 0; i--)
	{
		...
		{
			deploy& a_new_deploy = deploys.back();
			if (flavorsinfo.vcpus[i] <= a_new_deploy.cpuLeft && flavorsinfo.vmems[i] <= a_new_deploy.memLeft) //has space
			{
				a_new_deploy.nums[i]++;  //deploy
				a_new_deploy.cpuLeft -= flavorsinfo.vcpus[i]; //resources substract
				a_new_deploy.memLeft -= flavorsinfo.vmems[i];
			...
			else  //space not enough
			{
				...
				{
					if (flavorsinfo.vcpus[j] <= a_new_deploy.cpuLeft && flavorsinfo.vmems[j] <= a_new_deploy.memLeft) //has space
					{
						a_new_deploy.nums[j]++;  //deploy
						a_new_deploy.cpuLeft -= flavorsinfo.vcpus[j]; //resources substract
		...
		{
			//deploys.erase(deploys.end());
			//deploys.push_back(a_new_deploy);  //complete this deploy
		}
